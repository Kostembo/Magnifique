...
export function MobileTimesheetView({ employees, month, onEdit }: Props) {
  if (employees.length === 0) {
    return <p className="text-zinc-500 text-sm py-8 text-center">Нет отметок за этот месяц</p>;
  }
          ...
          <div key={empId} className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800/60">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-zinc-600">за месяц</p>
              </div>
            </div>

            {sortedDays.length === 0 ? (
              <p className="px-4 py-3 text-sm text-zinc-600">Нет записей</p>
            ) : (
