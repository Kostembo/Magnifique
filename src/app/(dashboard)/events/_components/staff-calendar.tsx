...
export function StaffCalendar({ shifts }: Props) {

  const calEvents = shifts.map((s) => ({
    id: s.event.id,
    title: s.event.title,
    start: s.event.starts_at,
    backgroundColor: "hsl(38,62%,48%)",
  ...
  return (
    <div className="fc-dark">
      <style>{`
        ...
        .fc-dark .fc-daygrid-event { border-radius: 4px; font-size: 0.72rem; padding: 1px 4px; cursor: default; pointer-events: none; }
        ...
        @media (max-width: 640px) {
          ...
          .fc-dark .fc-today-button .fc-icon, .fc-dark .fc-today-button { font-size: 0.7rem; padding: 0.3em 0.5em; }
        }
      `}</style>
      <FullCalendar
