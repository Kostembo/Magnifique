...
### Порядок работы через Pull Request (обязательный)

В `main` попадает только код, прошедший ревью напарника. Прямой push в `main` запрещён.

1. `git checkout main && git pull` — подтянуть свежий `main`.
2. `git checkout -b feat/короткое-имя` — ветка под задачу (`feat/`, `fix/`, `docs/`).
3. Работа + атомарные коммиты. Перед каждым коммитом — `npm run lint && npm run build` (оба зелёные).
4. `git push -u origin feat/короткое-имя` — отправить ветку на GitHub.
5. На GitHub открыть **Pull Request** в `main`. В описании — что и зачем менялось.
6. Напарник смотрит diff и жмёт **Approve** (лёгкое ревью — «глянул, вопросов нет» тоже ок).
7. После апрува — **Merge** (обычный merge или squash). Ветку после мержа удалить.
8. Оба делают `git checkout main && git pull` — снова синхронны.

**Если push отклонён** (*updates were rejected*): напарник запушил раньше → `git pull` в свою ветку, решить конфликты локально, снова `git push`.
